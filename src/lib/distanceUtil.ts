...
export const calcRouteByDirectionService = (address: AddressProps) =>
  new Promise((res) => {
    let distance = "";
    let final_distance = "";

    if (address.start === "" || address.start === "출발지" || address.start === "읍/면/동 주소") {
      return;
    }
    if (address.end === "" || address.end === "도착지" || address.end === "읍/면/동 주소") {
      return;
    }
...
export const calcRouteByGeoCoder = (addresses: string[]) =>
  new Promise((res) => {
