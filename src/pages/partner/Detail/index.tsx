...
const PartnerDetail = () => {
  ...
  useEffect(() => {
    if (getMoveIdxData.idx) {
      dispatch(
        partnerActions.fetchPartnerDetailAsync.request({
          adminId: params.adminId,
      ...
      {getPartnerDetail.data && (
        <>
          ...
          <S.PartnerInfoContainer>
            <PartnerInfo
              title={getPartnerDetail.data.title ? getPartnerDetail.data.title : values.DEFAULT_TEXT}
              level={getPartnerDetail.data.level}
              pick_cnt={getPartnerDetail.data.pick_cnt}
              experience={getPartnerDetail.data.experience}
              description={getPartnerDetail.data.description}
              keywords={getPartnerDetail.data.keywords}
              adminname={getPartnerDetail.data.adminname}
              addition={getPartnerDetail.data.addition}
            />
          </S.PartnerInfoContainer>
          <S.TitleContainer>
            <S.Wrap>
              <S.Box>
                <p>
                  고객들의 업체 평가는 위매치 약관에 의해 보호 받는 저작물로서, 무단복제 및 배포를 금합니다. <span onClick={toggleVisibleTerms}>자세히</span>
                </p>
              </S.Box>
              <S.Average>
                <strong>고객평가</strong>
          ...
          <S.ReviewContainer>{review()}</S.ReviewContainer>
          <S.BottomContainer>
            {getReviewList.moreLoading && <S.ReviewMoreLoading>로딩중..</S.ReviewMoreLoading>}
            {getReviewList.hasMore && (
              <S.MoreList onClick={handleMoreReview}>
                후기 더보기 <DownArrow width={16} height={16} />
              </S.MoreList>
            )}
            {showScrollView && (
              <S.ScrollView>
                <S.BtnSelect onClick={handleSelected} isSelected={isActive()} status={getPartnerDetail.data.status} id="dsl_booking_detail_cta">
                  {buttonText(getPartnerDetail.data.status)}
                </S.BtnSelect>
