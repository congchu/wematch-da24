...
const Card: React.FC<Props> = (props: Props) => {
    ...
    return (
      ...
      keywords.map((keyword, index) => {
        if (isMobile) {
          if (index < 3) {
            return (
              <div className="partner_keyword" key={index}>
  ...
  return (
    <>
      <S.CardContainer>
        <S.CheckBtn onClick={() => onSelect(list, adminid)} checkImage={require(`assets/images/check_circle_${isChecked ? "on" : "off"}.svg`)} />
        {profile_img ? (
        ...
        <S.Content onClick={() => setPartnerDetailVisible(!partnerDetailVisible)}>
          ...
          <S.KeywordWrapper>{keywordLength()}</S.KeywordWrapper>
        </S.Content>
      </S.CardContainer>
      <DetailPopup
        visible={partnerDetailVisible}
        onClose={() => {
