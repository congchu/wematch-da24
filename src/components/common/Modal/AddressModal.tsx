...
const AddressModal: React.FC<Props> = (props) => {
  const { visible = false, title, onClose, onClick, onSelect } = props;
  ...
  useEffect(() => {
    if (road.length > 1) {
      dispatch(
        commonActions.fetchAddressListAsync.request({
          keyword: road,
  ...
  useEffect(() => {
    ...
    return () => {
      document.removeEventListener("touchstart", keyboardOffEvent);
    };
  }, [visible]);
            ...
            <S.Empty>
              <p>
                {getAddressList.error?.code === "E0006" ? (
                  <>
                    <em>시도명</em>으로 검색할 수 없습니다.
                    <br />
                  </>
                ) : (
                  <>
                    <em>‘{road}'</em>검색 결과가 없습니다
                    <br />
                  </>
                )}
            ...
            <List
              ...
              onMoreAddresses={() => {
                if (getAddressList.hasMore && !getAddressList.loading) {
                  currPage.current++;
                  dispatch(
                    commonActions.fetchAddressMoreListAsync.request({
                      keyword: road,
                      currPage: currPage.current,
                      cntPerPage: CNT_PER_PAGE
                    })
