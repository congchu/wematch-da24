...
const S = {
  ...
  Button: styled.button<{buttonState: string}>`
    ...
    color: white;
    ${props =>
    props.buttonState === 'disabled' && css`
      background-color: ${lineDefault};
      cursor: default;
      ` ||
      props.buttonState === 'enabled' && css`
      background-color: ${pointBlue};
      ` ||
      props.buttonState === 'error' && css`
  ...
  Input: styled.input<{state?: 'error'}>`
    ...
    state === 'error' && `${error}` ||
      ...
      box-shadow: 0 4px 10px 4px ${props =>
    props.state === 'error' ? 'rgba(236, 72, 92, 0.1)' : 'rgba(45, 128, 247, 0.24)'
};
      background-color: white;
    }
    &:disabled {
...
export function ButtonBoxInput(props: Props) {
  ...
  return (
    <S.Container>
      <S.Input type={type} {...restProps} state={state}
        disabled={disabled} />
      <S.Button onClick={onClick} buttonState={buttonState} type="button"
        disabled={buttonState !== 'enabled'}>{buttonText}</S.Button>
    </S.Container>
  )
}
